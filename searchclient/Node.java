...
public class Node {
	...
	// E.g. Node.walls[2] is an array of booleans having size MAX_COL.
	// Node.walls[row][col] is true if there's a wall at (row, col)
	//
	...
	private Node ChildNode() {
		...
		for (int row = 0; row < MAX_ROW; row++) {
			System.arraycopy(Node.walls[row], 0, copy.walls[row], 0, MAX_COL);
			System.arraycopy(this.boxes[row], 0, copy.boxes[row], 0, MAX_COL);
			System.arraycopy(Node.goals[row], 0, copy.goals[row], 0, MAX_COL);
		}
		return copy;
	}
	...
	public boolean equals(Object obj) {
		...
		if (!Arrays.deepEquals(this.boxes, other.boxes))
			return false;
		if (!Arrays.deepEquals(Node.goals, other.goals))
			return false;
		if (!Arrays.deepEquals(Node.walls, other.walls))
			return false;
		return true;
	}
